...
/**
 * Oscar Vides
 * Function para cargar listado de tiquetes en espera
 * @param {type} str
 * @returns {undefined}
 */
...
function loadDataTable(str){
	 ...
	 $(function () {
         ...
         $('#table-javascript').bootstrapTable({
             method: 'get',
             url: window.location.protocol + "//" + window.location.host+'/colas/reimTiq/data/'+str+'',
             cache: false,
             height: 400,
             striped: true,
             pagination: true,
        ...
        var title = "<tr><th class='ui-widget-header' colspan="+i+">Reimpresión de Tiquete</th></tr>";
        var thead = title += enc;
        ...
        $('.keyboard-normal').keyboard({
            ...
            $.get(window.location.protocol + "//" + window.location.host+'/colas/reimTiq/data/'+($(this).val()===''?'Todos':$(this).val())+'',
                function(data){
...
window.operateEvents = {
    'click .re-print': function (e, value, row, index) {
    	if(row.estado==='En espera'){
    		paraImprimir(row.idTiquete);
    	}else{
    		mostrarAlertaError("Error","Reimpresión denegada","error");
    	}
