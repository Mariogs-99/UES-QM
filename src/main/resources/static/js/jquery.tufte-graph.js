(function($) {
  ...
  function addLegend(plot, options) {
    if (options.legend.dato) {
      elements = $(options.legend.dato).collect(function(i) {
  ...
  function makeAxis(options) {
    ...
    axis.x.min = 0
    axis.x.max = options.dato.length;
    axis.y.min = 0;
    axis.y.max = 0;

    $(options.dato).each(function() {
