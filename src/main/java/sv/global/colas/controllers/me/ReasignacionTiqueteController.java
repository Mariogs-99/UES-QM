...
@Controller
@EnableAsync
public class ReasignacionTiqueteController extends MainController {
    ...
    @RequestMapping(value="/reaTiq/data/{str}" , method=RequestMethod.GET, headers="Accept=application/json", produces="application/json")
    public @ResponseBody List<ReasignacionTiquetePojo> getData(@PathVariable(value="str") String str){
        ...
        try{	
            ...
            List<GcTiquete> tiquete = (List<GcTiquete>) gcTiqueteRepository.getListTiquetes(lUnidadRecep);
            ...
            for(GcTiquete t : tiquete){
                    ...
                    reaTiq.setPrioridad(t.getNPrioridadId().getNPrioridadId());
                    if(str.equals("Todos")){
