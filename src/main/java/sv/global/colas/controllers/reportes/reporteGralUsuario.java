...
@EnableAsync
public class reporteGralUsuario extends MainController{
    @Autowired
    ...
    public String getUsuariosXSeccion(HttpServletRequest request, 
            ...
            UsuarioPojo Usuariopojo = new UsuarioPojo();
            List usuarioList = gcUsuarioRepository.getUsuariosbyCS(unidad);
            Usuariopojo.setUsuarioList(setListToObjectUser(usuarioList));
            model.addAttribute("usuariopojo", Usuariopojo);
            model.addAttribute("unidadesList", new TbUnidadRecep());
        ...
        try{
            ...
            for(int i=0;i<userList.size();i++){
                ...
                for(int j=0;j<uList.length;j++){       
                    u.setCUsuario(uList[0].toString());
                }
                users.add(u);
            }
        }catch(Exception ex){
